...
A \textbf{multivariate Gaussian} (normal) distribution over 
$\mathbf{x}\in\mathbb{R}^d$ is defined by a mean vector 
$\boldsymbol{\mu}\in\mathbb{R}^d$ and a positive definite 
...
\begin{itemize}
    \item Elliptical level sets defined by the quadratic form  
    $(\mathbf{x}-\boldsymbol{\mu})^\top\boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})$.
    \item $\boldsymbol{\Sigma}$ encodes variances and pairwise correlations.
    \item Fully characterized by first and second moments.
\end{itemize}
...
\textbf{Interpretation of entries:}
\begin{itemize}
    \item Diagonal entries:  
    $\Sigma_{ii} = \mathrm{Var}(x_i)$  
    — how spread out the distribution is along dimension \(i\).
    
    \item Off-diagonal entries:  
    $\Sigma_{ij} = \mathrm{Cov}(x_i, x_j)$  
    — how dimensions \(i\) and \(j\) move together.
    
    \item Positive covariance:  
    \(x_i\) increases when \(x_j\) increases  
    \(\Rightarrow\) elongated ellipse along the diagonal direction.
    
    \item Negative covariance:  
    \(x_i\) increases when \(x_j\) decreases  
    \(\Rightarrow\) ellipse tilted in the opposite direction.
\end{itemize}

\bigskip
A \textbf{positive definite} $\boldsymbol{\Sigma}$ ensures a valid Gaussian with elliptical level sets.
\end{frame}
...
\begin{frame}{Multivariate Gaussian Distribution}
\centering
\includegraphics[width=0.65\linewidth]{pic/gaussians-3d.png}
\end{frame}

\begin{frame}{Multivariate Gaussian Distribution}
\centering
\includegraphics[width=0.65\linewidth]{pic/corr1.png}
\includegraphics[width=0.65\linewidth]{pic/corr2.png}
...
\begin{frame}{GMM}
\centering
\includegraphics[width=0.65\linewidth]{pic/mixture01.png}\par
\includegraphics[width=0.65\linewidth]{pic/mixture02.png}\par
\includegraphics[width=0.65\linewidth]{pic/mixture03.png}

\vfill
{\raggedright\tiny \textit{Source: Tilak Mudgal, “Gaussian Mixture Modeling (GMM),” Medium.}\par}
\end{frame}

\begin{frame}{Multivariate GMM}
\centering
\includegraphics[width=0.75\linewidth]{pic/mixture04.png}
...
\gamma_j^{(i)}
=
\mathbb{P}(z^{(i)}_j=1 \mid \mathbf{x}^{(i)}, \boldsymbol{\theta}^{(t)})
=
\frac{
\pi_j^{(t)}
\,
