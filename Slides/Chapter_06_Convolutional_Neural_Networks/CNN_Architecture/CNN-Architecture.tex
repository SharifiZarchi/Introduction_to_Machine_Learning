...
\begin{itemize}
    ...
    \item \color{red} Convolutional filters learn to make decisions based on local spatial input which is an important attribute working with images
\end{itemize}
\begin{figure}
    \centering
...
\begin{itemize}
    \item We apply the same filter on different regions of the input
    \item Convolutional filters learn to make decisions based on local spatial input which is an important attribute working with images.
...
\foreach \i in {0,...,8}
{
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.4\columnwidth]{images/kernel-frame-\i.png}
...
\begin{itemize}
    \item Polling layers aggregate the
...
\begin{itemize}
        \item{Polling layers goal is to subsample the input on order to Reduce:}
        \begin{itemize}
...
\begin{itemize}
    \item Max Pooling: give a better result when the background is white and the object is dark
    \item Min Pooling: give a better result when the background is dark and the object is white
...
\begin{center}
    \begin{figure}
        ...
        \caption{Different effects of different pooling layers}
        % \label{fig:testtt}
    \end{figure}
\end{center}
}
...
\begin{center}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\columnwidth]{images/Pooling_4.png}
...
\begin{itemize}
        \item{It is a technique that expands the kernel (input) by inserting holes between its consecutive elements}
\end{itemize}
\begin{center}
    \begin{figure}
        \centering
...
- As said before: "Filters always extend the full depth of the input volume. (\#Input channels == \#Filter Channels)"
\newline
\color{red}- Each filter results in one output channel, we can apply multiple different filters to get multiple output channels. Each channel can learn something different.
...
\frame{\frametitle{Channels, etc.}
\color{red}- We apply several filters and stack the output together to get a multilayer output, with each layer representing something it learnt.
\begin{center}
    \begin{figure}
        \centering
...
\newline
\color{red}- Some kernels learning to recognize vertical lines, some circles, and some, specific objects:
\begin{center}
    \begin{figure}
        \centering
