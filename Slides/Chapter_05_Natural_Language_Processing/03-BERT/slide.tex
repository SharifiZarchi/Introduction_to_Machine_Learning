...
\begin{frame}{Why Transformers?}
    ...
    \textbf{Pros:}
    \begin{itemize}
        \item[] {\color[rgb]{0,0.5,0}+ Much easier to parallelize than RNNs}
        \item[] {\color[rgb]{0,0.5,0}+ Much better at modeling long-range dependencies}
...
\begin{frame}{BERT: Key Contributions}
    ...
    \begin{itemize}
        ...
        This is because both left and right contexts are important to understand the meaning of words.}
...
\begin{frame}{Masked Language Modeling (MLM)}
    \begin{itemize}
        \item \textbf{Q:} Why can’t we do language modeling with bidirectional models?
    \end{itemize}
...
\begin{frame}{Fine-tuning BERT}
    ...
    \end{figure}
    For token-level prediction tasks, add a linear classifier on top of the hidden representations
...
\begin{frame}{Fine-tuning BERT}
    ...
    \end{figure}
    For sentence pair tasks, use [SEP] to separate the two segments with segment embeddings and add a linear classifier on top of the [CLS] representations
\end{frame}
...
\begin{frame}{Generative Pre-Trained Transformer (GPT)}
    ...
    \begin{itemize}
        ...
        \item Byte Pair Encoding (BPE) with 40,000 merges. 
