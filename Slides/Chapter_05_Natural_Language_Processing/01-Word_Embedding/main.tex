...
\begin{frame}{Skip-gram: Objective Function}
    \begin{itemize}
        ...
        \[
        P(w_o | w_c) = \frac{\exp(v_{w_o}^T v_{w_c})}{\sum_x \exp(v_x^T v_{w_c})}
        \]
...
\begin{frame}{Skip-gram: Loss Function}
    \begin{itemize}
        ...
        \]
        \item Here, \(T\) is the total number of words in the corpus, and \(m\) is the window size.
...
\begin{frame}{Skip-gram: Gradient Calculation}
    \begin{itemize}
        \item The detailed steps for the gradient calculation (derivative of the log-likelihood):
        \[
        \frac{\partial \log P(w_o | w_c)}{\partial v_{w_c}} = \frac{\partial}{\partial v_{w_c}} \log \frac{e^{v_{w_o}^T v_{w_c}}}{\sum_x e^{v_x^T v_{w_c}}}
        \]
        \[
        = \frac{\partial}{\partial v_{w_c}} \left( v_{w_o}^T v_{w_c} - \log \sum_x e^{v_x^T v_{w_c}} \right)
        \]
        \[
        = v_{w_o} - \frac{1}{\sum_x e^{v_x^T v_{w_c}}} \sum_x e^{v_x^T v_{w_c}} v_x
        \]
        \[
        = v_{w_o} - \sum_x P(w_x | w_c) v_x
        \]
        \item The update rule for \(v_{w_c}\) (Gradient Ascent):
        \[
...
\begin{frame}{Skip-gram Example with Gradient Calculation}
	\begin{itemize}
		...
		\item If the center word is $w_c = \text{"chase"}$, we predict context word $w_o = \text{"cats"}$.
		\item We calculate vectors to minimize the error between Observed (1) and Predicted probability ($P$).
		\item Example (simplified):
		\begin{itemize}
			\item Word vectors: \(v_{chase} = [0.1, 0.2]\), \(v_{cats} = [0.3, 0.4]\).
			\item Compute dot product: \(v_{cats}^T v_{chase} = 0.11\).
			\item Assume Softmax denominator sum is $2.264$.
			\item Probability: \(P(\text{"cats"} | \text{"chase"}) = \frac{\exp(0.11)}{2.264} \approx 0.493\).
		\end{itemize}
		\item Update Rule component: \(\text{Error} = (1 - 0.493) = 0.507\).
        \item We move \(v_{chase}\) closer to \(v_{cats}\) proportional to this error.
	\end{itemize}
\end{frame}
...
\begin{frame}{Skip-Gram Pseudocode}
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			...
			\State Initialize word embeddings $W$ and $C$ randomly
			...
			\]
			\State Calculate error $E = 1 - P(c|w_t)$ (for positive sample)
			\State Update $W(w_t)$ and $C(c)$ using gradient descent with rate \(\eta\)
			\EndFor
			\EndFor
			\EndFor
			\EndFor
