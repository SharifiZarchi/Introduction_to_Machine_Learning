...
\begin{frame}{Power-Law Scaling}
    ...
    \begin{itemize}
        \item
            \large{Loss and dataset size are linear on a log-log plot}\\
            \vspace{0.1cm}
        \item 
    ...
    \begin{itemize}
        ...
        \item
            ...
            \begin{outline}
                ...
                \1 Model scale is not the only contributor to emergence – for 14 BIG-Bench tasks, LaMDA 137B and GPT-3 175B models perform at near-random, but PaLM 62B achieves above-random performance
...
\begin{frame}{Attention patterns}
    ...
    \begin{itemize}
        ...
        \item In both the non-causal decoder and the encoder-decoder, attention is allowed to be bidirectional on any conditioning information.
...
\begin{frame}{Training of Decoder-only LLMs – Llama 2}
    \begin{itemize}
        ...
        \item Instruction Fine-tuning/ Supervised Fine-tuning (SFT) - Fine-tune the pretrained model with pairs of (instruction+input, output) with a large dataset and then with a small high-quality dataset
            ...
            \begin{itemize}
                \item  
                    ...
                    \begin{outline}
                        ...
                        \vspace{0.2cm}
                        \1 (b) PEFT instead learns a small subset of model parameters for each task with a fixed base LLM. The same base model can be reused during inference for different tasks.
                    \end{outline}
                    \vspace{0.3cm}
            \end{itemize}
        \end{column}
            ...
            \begin{itemize}
                \item  
                    \textbf{PEFT Baseline Efficiency:}
                     Freeze parameters except the top K layers, reducing computation and memory usage.
                    \vspace{0.3cm}
                \item  
                    \textbf{Flexible Application:}
                      It can be applied to most deep neural networks for efficient fine-tuning.
                    \vspace{0.3cm}
            \end{itemize}
        \end{column}
    ...
    \begin{columns} % Begin the columns environment
        ...
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                \item  
                    \textbf{Adapters: }
                     New modules are inserted between layers of a pre-trained model, with the original model weights fixed.
                    \vspace{0.3cm}
                \item  
...
\begin{frame}{Compacters}
            \begin{itemize}
                \item  
                    \textbf{Compacters: }
                     They are an extension of adapters, which aim to make the technique even more efficient.
                    \vspace{0.1cm}
                \item  
