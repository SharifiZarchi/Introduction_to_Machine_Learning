...
\begin{frame}{Gradient Descent: Concept and Weight Updates}
    ...
    \[
    \textbf{w}_{\text{new}} = \textbf{w}_{\text{old}} - \eta \cdot \frac{\partial L}{\partial \textbf{w}}
    \]
    Where:
    \begin{itemize}
        \item \( \eta \) is the learning rate (step size).
        \item \( \frac{\partial L}{\partial \textbf{w}} \) is the gradient of the loss function with respect to \( \textbf{w} \).
    \end{itemize}
\end{frame}
...
\begin{frame}{Recap: Problem Setup}

\begin{itemize}
    \item Given the architecture of the network and training data \(D = \{(x^{(1)}, y^{(1)}), \dots, (x^{(N)}, y^{(N)}) \}\), we aim to minimize the total loss.
    
    \begin{center}
        \[
        \textbf{$\hat{w}$} = \argmin_{\textbf{w}} E(\textbf{w})
        \]
    \end{center}
    
    \item To do so, we use the gradient descent method:
    \begin{center}
        \[
        \textbf{w}_{\text{new}} = \textbf{w}_{\text{old}} - \eta \cdot \frac{\partial L}{\partial \textbf{w}}
        \]
    ...
    \item How to calculate \(\frac{\partial L}{\partial \textbf{w}}\)?
\end{itemize}
\end{frame}
...
\begin{frame}{Notation Setup}
    The diagram below illustrates the \(l^\text{th}\) layer of a neural network.

    \vspace{0.3cm}
    \begin{figure}
        \centering
        \includegraphics[width=.8\linewidth]{pic/backprop_notation.png}
    \end{figure}
\end{frame}
...
\begin{frame}{Why Study Computational Gates?}

\begin{itemize}
    \item These simple gates (add, multiply, copy, max) are the 
    \textbf{fundamental building blocks} of all neural network computations.
    
    \item Any forward pass in a neural network can be decomposed into 
    combinations of these basic operations.
    
    \item Understanding each gate’s backward rule shows 
    \textbf{how gradients flow} through a model during backpropagation.
    
    \item These gates illustrate how gradients are:
    \begin{itemize}
        \item \textbf{Distributed} (add gate)
        \item \textbf{Scaled} (multiply gate)
        \item \textbf{Accumulated} (copy gate)
        \item \textbf{Routed} (max gate)
    \end{itemize}
    
    \item Mastering these rules builds intuition for 
    \textbf{how learning happens}, why gradients change, 
    and how models optimize their parameters.
\end{itemize}

\end{frame}


\begin{frame}{Chain Rule for Important Gates}
    \vspace{0.3cm}
    \begin{center}
        \includegraphics[width=0.8\linewidth]{pic/chain_rule_gates.png}\\[2mm]
        {\scriptsize Figure adopted from Fei-Fei Li, Jiajun Wu, and Ruohan Gao, \textit{CS231n}, Stanford, Fall 2022.}
    \end{center}
\end{frame}
...
\begin{frame}{Example: One Neuron}
    \begin{figure}
        ...
        {\tiny Image adapted from \href{https://datamapu.com/posts/deep_learning/backpropagation}{“Deep learning backpropagation” Web article}}
        \label{fig}
    \end{figure}
\end{frame}
...
\begin{frame}{Backward Pass in the lth Layer}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \vspace{-0.4cm}
            \[
            \frac{\partial L}{\partial w_{ji}^{[l]}} = \frac{\partial L}{\partial z_j^{[l]}}\frac{\partial z_j^{[l]}}{\partial w_{ji}^{[l]}} = \frac{\partial L}{\partial z_j^{[l]}} a_{i}^{[l-1]}
            \]
            \textbf{Note that:}
            \[
            z_{i}^{[l]} = \mathlarger{\mathlarger{\sum}}_{i=0}^{d[l-1]} w_{ji}^{[l]} a_i^{[l-1]}
            \]
            \[
            a_{i}^{[l]} = f(z_{i}^{[l]})
            \]
        ...
        \begin{column}{0.7\textwidth}
                    \begin{center}
                \includegraphics[width=\textwidth, keepaspectratio]{pic/backprop1.png}
            \end{center}
...
\begin{frame}{Backward Pass in the lth Layer}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \vspace{-0.4cm}
            \textbf{Now we need to calculate \(\frac{\partial L}{\partial z_j^{[l]}}\)}
            \[
            \frac{\partial L}{\partial z_i^{[l-1]}} = \frac{\partial L}{a_i^{[l-1]}} \frac{a_i^{[l-1]}}{z_i^{[l-1]}}
            \]
            \[
            = \frac{a_i^{[l-1]}}{z_i^{[l-1]}} \mathlarger{\mathlarger{\sum}}_{j=0}^{d[l]} w_{ji}^{[l]} \frac{\partial L}{\partial z_j^{[l]}}
            \]
        \end{column}
        ...
        \begin{column}{0.7\textwidth}
                    \begin{center}
                \includegraphics[width=\textwidth, keepaspectratio]{pic/backprop2.png}
            \end{center}
...
\end{itemize}
    \begin{figure}
        ...
        \vspace{2mm}
        {\tiny Image adapted from \href{https://www.researchgate.net/publication/363843256_A_Knowledge-driven_Physics-Informed_Neural_Network_model_Pyrolysis_and_Ablation_of_Polymers}{A Knowledge-driven Physics-Informed Neural Network model; Pyrolysis and Ablation of Polymers}}
        \label{fig}
    \end{figure}
\end{frame}
...
\begin{frame}{Variants of ReLU: Leaky ReLU}
    ...
    \end{itemize}
            ...
            \begin{tikzpicture}
                ...
                \addplot[color=red, thick, domain=-6:6] {max(0.1*x, x)};
                \end{axis}
            \end{tikzpicture}
            \end{center}
            \centering
...
\begin{itemize}
    ...
    \item \href{https://github.com/dibahadie}{Diba Hadi Esfangereh}
\end{itemize}
\end{itemize}
