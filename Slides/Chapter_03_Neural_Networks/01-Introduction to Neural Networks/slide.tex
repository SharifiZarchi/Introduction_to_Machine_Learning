...
\begin{frame}{Perceptron Reminder}
    ...
    \begin{minipage}{0.5\textwidth}
        \begin{figure}[htp]
            \centering
            % Use minipage blocks instead of the undefined `subfigure` environment
...
\begin{frame}{Single Hidden Layer Neural Network}
	...
	\begin{itemize}
		...
		\item Output layer: $$ o(\mathbf{x}) = \sigma^{(2)}\left(\mathbf{b}^{(2)} + \mathbf{W}^{(2)}h^{(1)}(\mathbf{x}) \right) $$
	\end{itemize}
	\endminipage
	\hfill
	\minipage{.4\textwidth}
