...
\begin{frame}{Modelling the Brain}
	...
	\begin{itemize}
		...
		\item \textbf{Dendrite:} Receives signals from other neurons neurons.
...
\begin{frame}{Example: Composing Neural Networks.}
    \begin{figure}[!htb]
        \minipage{0.5\textwidth}
          \includegraphics[width=\linewidth]{pic/5/nn_ex1.png}
        \endminipage\hfill
        \minipage{0.5\textwidth}
          \includegraphics[width=\linewidth]{pic/5/nn_ex2.png}
        \endminipage
    \end{figure}
\end{frame}
...
\begin{frame}{Single Hidden Layer Neural Network}
	...
	\begin{itemize}
		...
		\item Activated hidden layer: $$h(x) = \sigma^{(1)}(a(\mathbb{x}))$$
		\item Output layer: $$ o(\mathbb{x}) = \sigma^{(2)}\left(\mathbb{b}^{(2)} + \mathbb{W}^{(2)}h^{(1)}(\ma\mathbb{x}) \right) $$
	\end{itemize}
	\endminipage
	\hfill
	\minipage{.4\textwidth}
