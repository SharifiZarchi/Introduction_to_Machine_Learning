...
\SUTtopic{\textbf{Batch Normalization Fundamentals}}
\title{\SUTcourse}
\subtitle{\SUTseason}
\institute{\SUTaffiliation}
\date{}
...
\section{Batch Normalization Fundamentals}
...
\begin{enumerate}
    ...
    \]
    This allows the model to recover the original data distribution if necessary.
\end{enumerate}
\end{frame}
...
\begin{frame}
    ...
    \textbf{Note:} For simplicity, we assume $\mu_{\mathcal{B}}$ and $\sigma_{\mathcal{B}}$ are constant here to illustrate the scaling effect (in reality, they depend on the batch inputs):

    \begin{equation}
        \frac{\partial \mathcal{L}}{\partial x} \approx \frac{\partial \mathcal{L}}{\partial y} \cdot \frac{\partial y}{\partial \hat{x}} \cdot \frac{\partial \hat{x}}{\partial x}
    \end{equation}

    Where:
    \begin{itemize}
        \item \(\frac{\partial y}{\partial \hat{x}} = \gamma\)
        \item \(\frac{\partial \hat{x}}{\partial x} = \frac{1}{\sqrt{\sigma_{\mathcal{B}}^2 + \epsilon}}\)
    \end{itemize}

    Thus, the gradient becomes:
    \begin{equation}
        \frac{\partial \mathcal{L}}{\partial x} \approx \frac{\gamma}{\sqrt{\sigma_{\mathcal{B}}^2 + \epsilon}} \cdot \frac{\partial \mathcal{L}}{\partial y}
    \end{equation}
\end{frame}


\begin{frame}
    \frametitle{How Batch Normalization Smooths the Loss Landscape}
    ...
    \begin{equation}
        \frac{\partial \mathcal{L}}{\partial x} \text{ is scaled by } \frac{\gamma}{\sqrt{\sigma_{\mathcal{B}}^2 + \epsilon}}
    \end{equation}

    This consistent scaling results in a smooth loss landscape because:
    \begin{itemize}
        \item It stabilizes gradient flow, ensuring controlled optimization step sizes.
        \item Reduces risk of large oscillations or abrupt changes in the loss landscape.
        \item Makes the optimization process less likely to be trapped in local minima or saddle points.
    \end{itemize}
\end{frame}


\section{Batch Normalization in Practice}

\subsection{Inference Mode, Pros, Cons \& Implementation}

\begin{frame}{Batch Normalization in Training vs Inference}

    \textcolor{blue}{\textbf{Key Difference}}
    
    \begin{itemize}
        \item During \textbf{training}: Normalize using mini-batch statistics (mean and variance computed from current batch)
        \item During \textbf{inference}: Use running averages (exponential moving averages accumulated during training)
        \item \textbf{Why?} Inference typically uses single samples or small batches, so batch statistics are unreliable
    \end{itemize}
...
\begin{frame}{Key Takeaways: Batch Normalization}
    
    \begin{itemize}
        \item \textbf{Stabilization:} Reduces internal covariate shift and normalizes activations.
        \item \textbf{Acceleration:} Enables faster convergence with higher learning rates.
        \item \textbf{Regularization:} Acts as an implicit regularizer, reducing overfitting.
        \item \textbf{Scalability:} Facilitates training of deeper networks with less hyperparameter tuning.
        \item \textbf{Practical Impact:} One of the most important techniques in modern deep learning.
    \end{itemize}
