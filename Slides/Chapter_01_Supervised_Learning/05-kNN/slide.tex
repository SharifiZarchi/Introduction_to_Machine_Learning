...
\begin{itemize}
    \item \textbf{Given:}
    \begin{itemize}
        \item Training data $\{(\mathbf{x}^{(1)}, y^{(1)}), \dots, (\mathbf{x}^{(n)}, y^{(n)})\}$ are stored directly (no model parameters are learned).
    \end{itemize}
...
\begin{frame}{Cover \& Hart (1967)}
    ...
    \href{https://isl.stanford.edu/~cover/papers/transIT/0021cove.pdf}{\small\texttt{isl.stanford.edu/\string~cover/papers/transIT/0021cove.pdf}}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Computing the distance to a single example takes $O(d)$ time.
        \item Computing distances to all $n$ examples takes $O(nd)$ time.
        \item Selecting the $k$ nearest can be done in $O(n \log k)$ time using a \textbf{max-heap of size $k$}.
        \item Therefore, the total time complexity of kNN classification is:
        \[
            O(nd + n \log k).
        \]
        \item For fixed small $k$, this is effectively $O(nd)$.
    \end{itemize}

    \item This becomes prohibitively expensive when the number of samples and dimensions are both large.
\end{itemize}
...
\begin{columns}
    ...
    \begin{center}
        ...
        \[
        d_1(\mathbf{I}_1, \mathbf{I}_2) = \sum_i \left| \mathbf{I}_1^{(i)} - \mathbf{I}_1^{(i)} \right|.
        \]
    \end{center}
    ...
    \begin{center}
        ...
        \left( \sum_i (\mathbf{I}_1^{(i)} - \mathbf{I}_1^{(i)})^2 \right)^{\frac{1}{2}}.
        \]
    \end{center}
\end{columns}
...
\begin{columns}
    ...
    \begin{center}
        \textbf{$L_1$ (Manhattan) Distance:} \\[0.3em]
        \[
        d_1(\mathbf{I}_1, \mathbf{I}_2) = \sum_i \left| \mathbf{I}_1^{(i)} - \mathbf{I}_2^{(i)} \right|.
        \]
        \vspace{0.5em}
    ...
    \begin{center}
        ...
        \[
        d_2(\mathbf{I}_1, \mathbf{I}_2) =
        \left( \sum_i (\mathbf{I}_1^{(i)} - \mathbf{I}_2^{(i)})^2 \right)^{\frac{1}{2}}.
        \]
        \vspace{0.5em}
...
\textbf{Cosine Distance (Angle-Based Similarity)}
\begin{itemize}
    \item Measures the angle between vectors; scale-invariant.
    \item Often used for text or high-dimensional sparse data.
    \item Note: \textbf{Not a true metric} (triangle inequality may fail).
\end{itemize}
...
\begin{center}
    \includegraphics[width=0.42\textwidth]{pic/cosine.png}
\end{center}
