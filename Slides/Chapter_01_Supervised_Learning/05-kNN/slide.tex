...
\begin{frame}{Parametric vs. Non-Parametric Methods}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Learning: estimating parameters from data
...
\begin{minipage}{0.5\textwidth}
    \begin{itemize}
        ...
        \begin{itemize}
            \item The label of $x$ is predicted by \textbf{majority voting} among its $k$ nearest neighbors
        \end{itemize}
        \item Example: $k = 5$, $x = [x_1, x_2]$
    \end{itemize}
\end{minipage}%
...
\begin{itemize}
    \item \textbf{Given:}
    \begin{itemize}
        \item Training data $\{(x^{(1)}, y^{(1)}), \dots, (x^{(n)}, y^{(n)})\}$ are stored directly (no model parameters are learned)
    \end{itemize}

    \item \textbf{To classify a new sample} $x$:
    \begin{itemize}
        \item Find the $k$ nearest training samples to $x$
        \item Among these $k$ samples, count how many $k_j$ belong to each class $C_j$ ($j = 1, \dots, C$)
        \item Assign $x$ to the class $C_{j^*}$, where
        \[
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The data is \textbf{low-dimensional}
        \item A \textbf{small value of $k$} is used --- for instance, $k = 1$ depends only on the single nearest neighbor, which can be misleading in many test cases
    \end{itemize}
\end{itemize}
...
\begin{columns}
   \column{0.6\textwidth}
\begin{itemize}
    \item \textbf{Nearest Neighbor:} $k$-NN with $k = 1$
    \begin{itemize}
        \item Decision rule:
        \[
        \hat{y} = y^{NN(\mathbf{x})} \quad \text{where} \quad NN(\mathbf{x}) = \arg\min_{i=1,\dots,N} \left\| \mathbf{x} - \mathbf{x}^{(i)} \right\|
        \]
    \end{itemize}
    ...
    \begin{itemize}
        \item[] $R_n$: expected risk of the NN classifier with $n$ training examples drawn from $p(\mathbf{x}, y)$
        \vspace{0.3em}
...
Assume (although this is almost never the case) that we know $P(y|\mathbf{x})$  
...
We can compute the probability of that happening exactly, which gives the error rate

\[
\epsilon_{\text{BayesOpt}} = 1 - \mathbb{P}(y^*(\mathbf{x})|\mathbf{x}) 
= 1 - \mathbb{P}(y^*|\mathbf{x})
\]
...
\[
P(+1|\mathbf{x}) = 0.8 \qquad P(-1|\mathbf{x}) = 0.2
\]
...
The reason is that it provides a highly informative lower bound on the error rate  
Given the same feature representation, no classifier can achieve a lower error  
We use this fact to analyze the error rate of the $k$-NN classifier
...
\begin{itemize}
    \item As $n \to \infty$, each test point $x_t$ becomes almost identical to its nearest neighbor $x_{NN}$, so
    \[
    ...
    \]
    \item Thus, the 1-NN error is \textbf{at most twice} the Bayes optimal error
\end{itemize}
...
\begin{align*}
\epsilon_{NN} &= \mathbb{P}(y^*|\mathbf{x}_t)(1 - \mathbb{P}(y^*|\mathbf{x}_{NN})) + \mathbb{P}(y^*|\mathbf{x}_{NN})(1 - \mathbb{P}(y^*|\mathbf{x}_t)) \\
&\leq (1 - \mathbb{P}(y^*|\mathbf{x}_{NN})) + (1 - \mathbb{P}(y^*|\mathbf{x}_t)) \\ 
&= 2(1 - \mathbb{P}(y^*|\mathbf{x}_t)) = 2 \epsilon_{\text{BayesOpt}}
\end{align*}

where the inequality follows from $\mathbb{P}(y^*|\mathbf{x}_t) \leq 1$,  
and we assume that $\mathbb{P}(y^*|\mathbf{x}_t) = \mathbb{P}(y^*|\mathbf{x}_{NN})$
...
\begin{itemize}
    ...
    \item The volume of a single ball with radius $\epsilon$ is $\mathcal{O}(\epsilon^d)$
    \item The total volume of $[0,1]^d$ is 1
    \item Therefore, $\mathcal{O}\!\left(\left(\tfrac{1}{\epsilon}\right)^d\right)$ balls are needed to cover the entire volume
\end{itemize}
...
\textbf{Observation:}  
As $d$ increases, almost the entire space is needed to find the $k$ nearest neighbors, breaking the fundamental assumption of kNN
...
This breaks the fundamental assumption underlying kNN
...
\begin{itemize}
    ...
    \item As we increase $k$, the model becomes smoother and less complex
\end{itemize}
...
\begin{itemize}
    \item In practice:
    \begin{itemize}
        \item \textit{k} should be large enough to minimize the overall error rate
        \begin{itemize}
            \item A very small \textit{k} produces noisy and unstable decision boundaries
        \end{itemize}
        \item \textit{k} should also be small enough to ensure that only nearby samples are taken into account
        \begin{itemize}
            \item A very large \textit{k} produces overly smooth boundaries that may ignore local patterns
        \end{itemize}
    \end{itemize}
...
\begin{itemize}
    \item For $k = 1, \ldots, 7$, the point $\mathbf{x}$ is correctly classified as belonging to the red class
    \item For larger values of $k$, the classification of $\mathbf{x}$ becomes incorrect and it is assigned to the blue class
    \item Bias–Variance Tradeoff in kNN:
    \begin{itemize}
...
\begin{itemize}
    \item $k$ is an example of a hyperparameter --- a parameter that cannot be directly learned during the training process
...
\begin{itemize}
    \item Main components required to construct an instance-based learner:
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The Minkowski distance is also equal to the $L^p$ norm of $(x - x')$
    \end{itemize}
...
\[
d(x, x') = \sqrt{(x_1 - x'_1)^2 + \cdots + (x_d - x'_d)^2}
\]
...
\begin{center}
    \includegraphics[width=0.85\textwidth]{pic/l1l2.png}
\end{center}
...
\begin{align*}
    ...
    \text{where} \quad 
    \text{cosine similarity}(x, x') &= 
    \frac{x \cdot x'}{\|x\|_2 \, \|x'\|_2} 
...
\begin{figure}[h]
    ...
    \caption*{\scriptsize Example of angular difference in cosine similarity}
\end{figure}
...
You can get quite creative with the choice of the distance function  
For example, using \textit{bending energy}, which measures how much transformation is required to make one example resemble another

\begin{center}
    \includegraphics[width=1\textwidth]{pic/s5.png}
\end{center}
...
\begin{itemize}
    \item Let $x'^{(1)}, \dots , x'^{(k)}$ be the $k$ nearest neighbors of $x$, and let $y'^{(1)}, \dots, y'^{(k)}$ be their corresponding labels
    \[
    ...
    \begin{itemize}
        ...
        \item For 1-NN: prone to noise fitting
        \item For $k > 1$: averaging smooths out noise but may introduce other issues (e.g., flattening near the boundaries)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \[
        Y = 2 \sin(X) + \epsilon
    \]
    \item Where $\epsilon$ is a standard normal error term
    \item We simulate 200 observations and visualize the model for $k = 1$
\end{itemize}
...
\begin{itemize}
    \item As $k$ increases, the model becomes smoother. However, if $k$ is too large, it starts to deviate from the true underlying function
\end{itemize}
...
\begin{columns}
    ...
    \column{0.32\textwidth}
    \includegraphics[width=\textwidth]{pic/function2.png}
...
\begin{itemize}
    \item For each test sample, it produces a linear approximation of the target function in a local region
    \item Instead of computing the output by weighted averaging (as in kernel regression), we fit a parametric function locally
\end{itemize}

\vspace{0.5em}

\[
\hat{y} = f\left(x, x^{(1)}, y^{(1)}, \ldots, x^{(n)}, y^{(n)}\right)
\]

\[
\hat{y} = f(x; \mathbf{w}) = w_0 + w_1 x_1 + \cdots + w_d x_d
\]

\vspace{0.5em}

\[
J(\mathbf{w}) = \sum_{i \in \mathcal{N}_k(x)} \left( y^{(i)} - \mathbf{w}^T x^{(i)} \right)^2
\]
