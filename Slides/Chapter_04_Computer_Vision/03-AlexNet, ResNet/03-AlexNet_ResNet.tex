...
\begin{frame}{Vanishing Gradient}
	\begin{itemize}
		...
		\item which can clearly explode or vanish unless $w$ is very close to $1$. For instance, if $w=1.1$ and $T=50$, then $\partial h^{(T)}/\partial h^{(1)} \approx 117.4$, whereas if $w=0.9$ and $T=50$, it shrinks rapidly.
