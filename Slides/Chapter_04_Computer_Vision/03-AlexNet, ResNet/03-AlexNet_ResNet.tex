...
\begin{frame}{Challenge of Making CNNs Deeper}
	\begin{itemize}
		\item \textbf{Fact:} Deeper models have \textbf{greater representational power} (i.e., more parameters) than shallower models.
		\item However, a \textbf{56-layer model performs worse} on both training and test error than a shallower model, and this is \textbf{not due to overfitting}.
		\item \textbf{Hypothesis:} The difficulty is related to optimization: deeper models are harder to train.
	\end{itemize}
	\vspace{-0.4cm}
	\centering
	\includegraphics[keepaspectratio, scale=0.23]{pic/vanish.png}
\end{frame}


\begin{frame}{Why Deep Networks Are Hard to Train}
\begin{itemize}
	\item Increasing depth increases a network's \textbf{representational power}.
	\item However, simply stacking more layers often leads to:
	\begin{itemize}
		\item slow or stalled training,
		\item poor training accuracy,
		\item worse performance than shallower networks.
	\end{itemize}
	\item This issue is primarily an \textbf{optimization problem}, not overfitting.
	\item A key difficulty is unstable \textbf{gradient propagation} in deep networks.
\end{itemize}
\end{frame}


\begin{frame}{Backpropagation Through a Single Layer}
\begin{itemize}
	\item Consider a deep network without skip connections.
	Let
	\[
		z^{(t)} = w\, a^{(t-1)}, \qquad
		a^{(t)} = \phi\!\bigl(z^{(t)}\bigr).
	\]

	\item Using the chain rule, the gradient propagates as
	\begin{equation}
		\frac{\partial L}{\partial a^{(t-1)}} =
		\frac{\partial L}{\partial a^{(t)}}
		\cdot w
		\cdot \phi'\!\bigl(z^{(t)}\bigr).
	\end{equation}

	\item Each layer \textbf{scales} the gradient by
	\( w \, \phi'(z^{(t)}) \).
\end{itemize}
\end{frame}


\begin{frame}{Gradient Propagation Across Many Layers}
\begin{itemize}
	\item Repeating this process across \( T \) layers yields
	\begin{equation}
	...
	\item The gradient becomes a \textbf{product of many terms}.
	\item Even small deviations from \( 1 \) are amplified exponentially with depth.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\begin{itemize}
		\item If \( |w\,\phi'(z)| < 1 \) on average,
		\item gradients shrink exponentially as depth increases,
	...
	\item In the multivariate case, this behavior is governed by the
