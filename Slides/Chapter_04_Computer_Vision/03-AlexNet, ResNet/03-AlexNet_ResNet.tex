...
\begin{itemize}
    ...
    \[
        h^{(t)} \in \mathbb{R}^d
    \]
    Let this denote the \textbf{hidden activation} at layer $t$.
    
    \[
        z^{(t)} = W h^{(t-1)}
    \]
    and this be the \textbf{pre-activation} at layer $t$, where
    \[
    ...
    \]
    The activation function is that with element-wise derivative $\phi'(z^{(t)})$.
    
    \[
        \bar{h}^{(t)} \triangleq \frac{\partial \mathcal{L}}{\partial h^{(t)}}
    \]
    And this denotes the \textbf{backpropagated error signal} at layer $t$.
\end{itemize}
...
\begin{frame}{Vanishing Gradient}
	\begin{itemize}
		\item Now consider a deep neural network without residual blocks
...
\begin{frame}{Jacobian}
	...
	\item More generally, in the multivariate case,
	\begin{equation}
