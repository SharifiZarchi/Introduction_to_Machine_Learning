...
\begin{document}
	...
	\begin{frame}{Channels}
		...
		\begin{itemize}
			...
			\item It is not possible to represent all colors in a picture using only a single channel with numbers ranging from 0 to 255.
	...
	\begin{frame}{Pooling}
		\begin{itemize}
			\item Convolution and activation layers are often followed by pooling layers at intervals.
    ...
    \begin{itemize}
        \item \textbf{Goal:} Stabilize and accelerate neural network training by normalizing activations.
        \textit{(typically after the linear/convolution layer and before activation.)}
        \item \textbf{Key Idea:} Reduce internal covariate shift by normalizing feature statistics.
        \item \textbf{Types of Normalization:}
        \begin{itemize}
        ...
        \end{itemize}
        \item Each method differs by \textit{which dimensions} are normalized and how mean ($\mu$) and variance ($\sigma$) are computed.
    \end{itemize}
    \end{frame}
    
    \begin{frame}
    \frametitle{Batch Normalization: Training Time vs. Test Time}
    ...
    \textbf{Inference:}
    \begin{itemize}
        \item Use running averages of $\mu_j$ and $\sigma_j^2$ instead of batch statistics.
        \item This ensures stable predictions even with batch size = 1.
    \end{itemize}
    ...
    \begin{columns}[T]
        ...
        \centering
        \textbf{Batch Normalization for fully-connected networks}
        \[
        \begin{aligned}
        \mathbf{x} &: N \times D \\
        \boldsymbol{\mu}, \boldsymbol{\sigma} &: 1 \times D \\
        \boldsymbol{\gamma}, \boldsymbol{\beta} &: 1 \times D \\
        \mathbf{y} &= \boldsymbol{\gamma} \frac{\mathbf{x} - \boldsymbol{\mu}}{\boldsymbol{\sigma}} + \boldsymbol{\beta}
        \end{aligned}
        \]
        
        \column{0.48\textwidth}
        \centering
        \textbf{Batch Normalization for convolutional networks}
        \[
        \begin{aligned}
...
\begin{frame}
    ...
    \begin{columns}[T]
        \column{0.48\textwidth}
        \centering
        \textbf{Layer Normalization for fully-connected networks}\\
        \textit{Same behavior at train and test.}\\
        \textit{Can be used in recurrent networks.}
        \[
        \begin{aligned}
        \mathbf{x} &: N \times D \\
        ...
        \column{0.48\textwidth}
        \centering
        \textbf{Layer Normalization for convolutional networks}\\
        \textit{Applies normalization over all channels and spatial locations per sample.}\\
        \[
        \begin{aligned}
        \mathbf{x} &: N \times C \times H \times W \\
        \boldsymbol{\mu}, \boldsymbol{\sigma} &: N \times 1 \times 1 \times 1 \\
        \boldsymbol{\gamma}, \boldsymbol{\beta} &: 1 \times C \times 1 \times 1 \\
        \mathbf{y} &= \boldsymbol{\gamma} \frac{\mathbf{x} - \boldsymbol{\mu}}{\boldsymbol{\sigma}} + \boldsymbol{\beta}
        \end{aligned}
        \]
...
\begin{frame}
    \frametitle{Instance Normalization}
    \begin{center}
        \textbf{Instance Normalization for Convolutional Networks} \\
...
\end{frame}
    ...
    \begin{frame}
    \frametitle{Group Normalization}

    \begin{center}
        \textbf{Group Normalization for Convolutional Networks} \\
    ...
    \boldsymbol{\mu}, \boldsymbol{\sigma} &: N \times G \times 1 \times 1 \\
    \boldsymbol{\gamma}, \boldsymbol{\beta} &: 1 \times C \times 1 \times 1 \\
    \mathbf{y} &= \boldsymbol{\gamma} \frac{\mathbf{x} - \boldsymbol{\mu}}{\boldsymbol{\sigma}} + \boldsymbol{\beta}
    \end{aligned}
    \]
    ...
    \begin{frame}{Comparison of Normalization Strategies}
    \centering
    \includegraphics[
        width=\linewidth,
        height=0.85\textheight,
        keepaspectratio
    ]{pic/normalizations.jpg}
    \end{frame}
	...
	\begin{frame}{Power Of Small Filters}
		...
		\bigskip
		\begin{flushleft}
			Both configurations achieve a receptive field of 7. However, using \textbf{multiple smaller filters} reduces the number of \textbf{parameters}, introduces more \textbf{nonlinearity}, and generally leads to a \textbf{more efficient}, expressive model.
		\end{flushleft}
	\end{frame}	
	\begin{frame}{Question}
